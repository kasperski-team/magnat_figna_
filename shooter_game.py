...
display.set_caption("магнат шрэка 3, ай лэв🦁")
clock = time.Clock()
...
# Текст
win_text = font_game.render("ты нихуя себе выйграл, ай лэв ;)", True, GREEN)
lose_text = font_game.render("ты проебал, :(", True, RED)
# Статы
score = 0
lost = 0
life = 13
...
start_button = Button("ооооочень большая кнопка, нажми чтоб начать игру, ай лэв :)", WIDTH // 2 - 100, HEIGHT // 2 - 60, 200, 80)
exit_button = Button("оооооень большая кнопка, нажми чтоб выйти, ай лэв :)", WIDTH // 2 - 100, HEIGHT // 2 + 40, 200, 80)
...
while run:
    for e in event.get():
        if e.type == QUIT:
            run = False
        if e.type == KEYDOWN:
            if e.key == K_SPACE and not finish and not menu:
                if player.current_bullets != 0 and not rel_time:
                    player.fire()
                    play_fire.play()
                else:
                    last_time = timer() # from time import time as timer
                    rel_time = True
